...
import { Package, Users, LogOut, MapPin, Plus, Edit2, Check, Download, Upload, Share2, FileSpreadsheet, Calendar, X, Settings } from 'lucide-react';
  ...
  const handleShareWhatsApp = async () => {
    try {
      ...
      deliveryList.forEach(item => {
        if (!byCustomer[item.customer_id]) {
          byCustomer[item.customer_id] = {
        ...
        byCustomer[item.customer_id].products.push({
      ...
      let message = `*📋 Delivery List - ${filters.delivery_date}*\n`;
      ...
      customers.forEach((customer, index) => {
        customer.products.forEach((product, pIndex) => {
          const rowNum = pIndex === 0 ? String(index + 1).padEnd(2) : '  ';
          const custName = pIndex === 0 ? customer.name.substring(0, 13).padEnd(13) : ' '.repeat(13);
          const phone = pIndex === 0 ? customer.phone.substring(0, 10).padEnd(10) : ' '.repeat(10);
          const productName = product.name.substring(0, 14).padEnd(14);
          const qty = String(product.qty).padEnd(4);
          const area = pIndex === 0 ? customer.area.substring(0, 15) : '';

          message += `${rowNum} | ${custName} | ${phone} | ${productName} | ${qty} | ${area}\n`;
        });
      });
      ...
      toast.success(`✓ Copied ${customers.length} deliveries in table format!`);
    } catch (error) {
      console.error('WhatsApp copy error:', error);
            ...
            <div className="flex items-center gap-2">
              ...
              {isAdmin && (
                <>
                  ...
                  <Button onClick={() => navigate('/admin/inventory')} className="bg-indigo-600 hover:bg-indigo-700" size="sm">
                    <FileSpreadsheet className="mr-2 h-4 w-4" />Inventory
                  </Button>
        ...
        <Tabs defaultValue="customers">
                      ...
                      <div key={product._id} className="p-4 border rounded-lg hover:bg-gray-50">
                        <div className="flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">Price: ₹{product.price_per_unit}/{product.unit}</p>
                          </div>
          ...
          {isAdmin && (
            ...
            <TabsContent value="deleted">
              <Card>
                <CardHeader>
                  <CardTitle className="text-red-600">Deleted Customers</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {deletedCustomers.length > 0 ? (
                      deletedCustomers.map(customer => (
                        <div key={customer._id} className="p-4 border border-red-200 rounded-lg bg-red-50">
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="font-bold text-lg">{customer.name}</h3>
                              <p className="text-sm text-gray-600">{customer.phone}</p>
                              <p className="text-sm text-gray-600">{customer.address}, {customer.area}</p>
                              <p className="text-xs text-red-600 mt-1">Deleted on: {new Date(customer.deleted_at || Date.now()).toLocaleDateString()}</p>
                            </div>
                          </div>
                        </div>
                      ))
            ...
            <TabsContent value="requests">
              <Card>
                <CardHeader>
                  <CardTitle>Pending Requests from Shared Links</CardTitle>
                </CardHeader>
                <CardContent>
                  {pendingRequests.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {pendingRequests.map(request => (
                        <div key={`${request.request_type}-${request.customer_id}`} className="border rounded-lg p-4 bg-white hover:shadow-md transition">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <h3 className="font-semibold text-lg">{request.customer_name}</h3>
                                <span className={`px-2 py-1 text-xs rounded font-semibold ${
                                  request.request_type === 'product' ? 'bg-blue-100 text-blue-800' :
                                  request.request_type === 'pause' ? 'bg-amber-100 text-amber-800' :
                                  'bg-red-100 text-red-800'
                                }`}>
                                  {request.request_type === 'product' ? '➕ Add Product' :
                                   request.request_type === 'pause' ? '⏸ Pause' :
                                   '🛑 Stop'}
                                </span>
                              ...
                              {request.request_type === 'product' && (
                                <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                                  <div>
                                    ...
                                    <span className="ml-2 font-medium">{request.product_id}</span>
                                  </div>
                                  <div>
                                  ...
                                  <div>
                                    ...
                                    <span className="ml-2 font-medium">
                                      {request.delivery_date ? new Date(request.delivery_date).toLocaleDateString() : 'Whenever Available'}
                                    </span>
                                  </div>
                                  <div>
                                    <span className="text-gray-600">Via:</span>
                                    <span className="ml-2 font-medium">{request.requested_via === 'shared_link' ? 'Shared Link' : 'Direct'}</span>
                                  </div>
                                </div>
                              )}

                              {(request.request_type === 'pause' || request.request_type === 'stop') && (
                                <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                                  <div>
                                    <span className="text-gray-600">Reason:</span>
                                    <span className="ml-2 font-medium capitalize">{request.reason}</span>
                                  </div>
                                  <div>
                                    <span className="text-gray-600">Via:</span>
                                    <span className="ml-2 font-medium">{request.requested_via === 'shared_link' ? 'Shared Link' : 'Direct'}</span>
