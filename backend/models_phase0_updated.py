...
class Customer(BaseModel):
    model_config = ConfigDict(extra="ignore")
    id: str
    user_id: Optional[str] = None  # STEP 21: Link to db.users for authentication
    name: str
    phone: str
    address: str
    area: str
...
class CustomerCreate(BaseModel):
    user_id: Optional[str] = None  # STEP 21: Optional - can be added later or during creation
    name: str
    phone: str
    address: str
    area: str
...
class CustomerUpdate(BaseModel):
    user_id: Optional[str] = None  # STEP 21: Can update linking
...
# STEP 26: Item-level quantity tracking model
class DeliveryItem(BaseModel):
    product_id: str
    product_name: str
    ordered_qty: float  # How many were ordered
    delivered_qty: float  # How many were actually delivered
    status: str  # "full" (delivered_qty == ordered_qty), "partial" (0 < delivered_qty < ordered_qty), "shortage" (delivered_qty == 0)
    price_per_unit: Optional[float] = None
...
class DeliveryStatus(BaseModel):
    ...
    confirmed_by_user_id: Optional[str] = None  # User who confirmed delivery (null if shared link)
    confirmed_by_name: Optional[str] = None  # Name of user who confirmed (null if shared link)
    confirmed_at: Optional[str] = None  # ISO timestamp when confirmed
    confirmation_method: Optional[str] = None  # "delivery_boy", "shared_link", or "admin"
    ip_address: Optional[str] = None  # IP address from shared link access
    device_info: Optional[str] = None  # User agent from shared link access
    # STEP 26: Item-level quantity tracking
    items: Optional[List[DeliveryItem]] = None  # Items with delivered vs ordered quantities
