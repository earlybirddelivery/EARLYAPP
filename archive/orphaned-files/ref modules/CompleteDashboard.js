...
import { Package, Users, LogOut, MapPin, Plus, Edit2, Check, Download, Upload, Share2, FileSpreadsheet, Calendar, X } from 'lucide-react';
...
export function CompleteDashboard() {
  ...
  const fetchWalletData = async () => {
    try {
      ...
      const res = await api.get(`/phase0-v2/customers/${currentUser.id}`);
  ...
  const handleTopUpSubmit = async () => {
    ...
    try {
      ...
      toast.success('Payment link opened. Complete the payment to top-up wallet.');
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="px-4 py-2 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-200">
                <div className="text-xs text-gray-600 font-medium">Wallet Balance</div>
                <div className="text-xl font-bold text-blue-600">₹{walletBalance?.toFixed(2) || '0.00'}</div>
              </div>
              <Button onClick={() => setShowTopUpDialog(true)} className="bg-green-600 hover:bg-green-700" size="sm">
                <Plus className="mr-2 h-4 w-4" />Top-up
              </Button>
              <Button onClick={() => navigate('/monthly-billing')} variant="default" size="sm">
      ...
      <Dialog open={showTopUpDialog} onOpenChange={setShowTopUpDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Current Balance: ₹{walletBalance?.toFixed(2) || '0.00'}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex gap-2 flex-wrap">
                {[100, 250, 500, 1000, 2000].map(amt => (
                  <button
                    ...
                    className="px-3 py-1 bg-blue-200 hover:bg-blue-300 rounded text-sm font-medium transition"
                  >
                    ₹{amt}
                  </button>
                ))}
          ...
          <DialogFooter>
            <Button onClick={() => setShowTopUpDialog(false)} variant="outline">
              Cancel
            </Button>
            <Button 
